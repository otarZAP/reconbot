...
wheels through an **L298N** motor driver, with an **MG90S servo** tilting the
camera to "look up" and scan a room.
...
* **Tilting camera head** — one servo tilts the camera up/down to scan a room.
...
| MG90S micro servo | 1 | tilts the camera |
...
| MT3608 boost | 1 | 5 V for motors + servo |
...
18650 ─► TP4056 (charge/protect) ─► switch ─┬─ TPS63020 → 3.3V → ESP32-S3-CAM (brain)
                                            └─ MT3608   → 5.0V → L298N + MG90S servo (muscle)
```
...
4. **Connect your phone** — join WiFi **`ReconBot`** (default password
   ...
   **`http://192.168.4.1`**.
5. **Drive** — touch-hold the D-pad, slide for speed and camera tilt.
